...
import {
  ...
  editDestination,
  removeDestination,
} from "../destinations.slice";

const { Title } = Typography;

const DestinationsPage = () => {
  const dispatch = useAppDispatch();
  const { data, loading } = useAppSelector((state) => state.destinations);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRecord) {
        await dispatch(
          editDestination({
            id: editingRecord.destinationId,
            data: values,
          })
        ).unwrap();
        message.success("Destination updated successfully");
      } else {
        await dispatch(addDestination(values)).unwrap();
  ...
  const handleDelete = async () => {
    ...
    try {
      await dispatch(removeDestination(deletingRecord.destinationId)).unwrap();
